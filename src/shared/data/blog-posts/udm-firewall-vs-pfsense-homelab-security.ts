...
- No per-rule logging or hit counts exposed in the UI, so you can't audit what's actually firing or how often
- No firewall state table visibility, so there's no way to see active connections per rule
- No packet capture filtering by IP range or protocol, which limits forensic capability when something goes wrong
- QoS/traffic shaping causes ~30% throughput reduction when enabled
- GeoIP blocks are all-or-nothing, with no per-port country filtering (can't block RU/CN on SSH only)
...
| :--- | :--- | :--- |
| CVE-2026-22557 | **10.0 Critical** | Path traversal in UniFi Network Application, letting an attacker access files on the underlying system |
...
1. **UniFi gateway**, properly configured: Threat Management enabled (check your device has IDS/IPS, since the original Express does not), GeoIP filtering active for countries you have no business receiving traffic from, VLAN segmentation isolating IoT from trusted networks.
...
1. **OPNsense as primary firewall**, replacing the UniFi gateway's routing role. Keep UniFi hardware (switches, APs) downstream; they work without a UniFi gateway.
